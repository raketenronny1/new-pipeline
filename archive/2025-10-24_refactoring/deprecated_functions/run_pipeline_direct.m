%% RUN_PIPELINE_DIRECT - Direct pipeline execution (DEPRECATED)
%
% ⚠️ DEPRECATED: This function is deprecated as of Version 4.0 (October 2025)
% ⚠️ Please use run_pipeline() instead
% ⚠️ 
% ⚠️ Migration:
% ⚠️   Old: run_pipeline_direct(true)
% ⚠️   New: run_pipeline('OutlierMethod', 'qc')
%
% This file has been backed up to: archive/2025-10-24_refactoring/
%
...
function run_pipeline_direct(perform_qc)
    ...
    warning('run_pipeline_direct:deprecated', ...
            ['run_pipeline_direct() is DEPRECATED as of Version 4.0 (October 2025).\n' ...
             'Please use run_pipeline() instead.\n' ...
             'Migration: run_pipeline(''OutlierMethod'', ''qc'')']);
    
    fprintf('\n');
    fprintf('╔════════════════════════════════════════════════════════════╗\n');
    fprintf('║  ⚠️  DEPRECATION WARNING                                  ║\n');
    fprintf('║                                                            ║\n');
    fprintf('║  This function is deprecated. Please use:                 ║\n');
    fprintf('║  >> run_pipeline(''OutlierMethod'', ''qc'')                  ║\n');
    fprintf('║                                                            ║\n');
    fprintf('║  Continuing with legacy code for now...                   ║\n');
    fprintf('╚════════════════════════════════════════════════════════════╝\n');
    fprintf('\n');
    pause(2);  % Give user time to see the warning
