function perform_feature_selection(cfg)
    ...
    fprintf('DEBUG: Fields in trainingData: ');
    fields = fieldnames(trainingData);
    fprintf('%s ', fields{:});
    fprintf('\n');
    
    % Check and fix the y field format if needed
    if ~isfield(trainingData, 'y')
        error('Training data does not have a y field. Data structure is incomplete.');
    end
    
    % Directly call our implementation
    perform_feature_selection_fixed(cfg);
end

function perform_feature_selection_original(cfg)
    %% Load Preprocessed Data
    fprintf('Loading preprocessed data...\n');
    load(fullfile(cfg.paths.results, 'preprocessed_data.mat'), 'trainingData');
    ...
    % Save models and transformed data
    save(fullfile(cfg.paths.results, 'X_train_pca.mat'), 'X_train_pca_reduced');
